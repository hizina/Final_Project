	...
	<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
		<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
			...
			<div class="container-fluid">
					...
					<div class="col-md-6" style="height: 60vh;">
						<div class="card">
							<div class="card-header" style="padding-bottom:10px;">
								<h4>
									<span style="font-size: 20px; color: #f96d00;">|</span><strong>&nbsp;휴학/복학신청</strong> 
								</h4>
							</div>
							<div class="card-body">
								<!--  휴학신청 공지사항  -->
								<c:if test="${empty acVO }">
									<div class="card-body pb-3 pt-0"
										style="background-color: #eb060633; border-radius: 5px;">
										...
										<div class="row" style="margin-top: 20px;">
											...
											<div class="col-md-4">
												<label class="col-lg-4 col-form-label">성명</label> <input type="text"
													...
													class="form-control" readonly /> <label
													class="col-lg-6 col-form-label"> 이수학기</label> <input type="text"
					...
					<c:forEach items="${acVO }" var="acVO" varStatus="i">
						<c:choose>
							<c:when test="${acVO.acStatus eq 'Y'}">
								...
								<c:set var="text" value="승인내역이 존재합니다." />
							</c:when>
							<c:when test="${acVO.acStatus eq 'W'}">
								<c:set var="disabled" value="disabled" />
								<c:set var="display" value="none" />
								<c:set var="text" value="대기내역이 존재합니다." />
							</c:when>
						</c:choose>
					</c:forEach>
					<div class="col-md-6">
						<div class="card" >
							<div class="card-header" style="padding-bottom:10px;">
								<h4>
									<span style="font-size: 20px; color: #f96d00;">|</span><strong>&nbsp;필수입력사항</strong>
								</h4>
							</div>
							<div class="">
									...
									<div class="row">
										<div class="card-body">
											<div class="card-body pb-3 pt-3"
												...
												<code style="background-color: #ffe2e200; color: red; font-size: 18px;display:">*</code>
				...
				<div class=card>
					<div class="card-header" style="padding-bottom:10px;">
							<h4><span style="font-size: 20px; color: #f96d00;">|</span><strong>&nbsp;신청내역</strong></h4>
					</div>
					<div class="" >
							...
							<div>
								<c:if test="${status eq 'ac' }">
									<c:choose>
										...
										<c:otherwise>
											<table id = "infoTable" class="table table-responsive table-bordered text-center">
												<thead>
						...
						$("#chgBtn").click(function () {
							...
							$.ajax({
								  ...
								  success: function (res) {
								    ...
								    $("#infoContent").empty();
								    // 새로운 테이블 데이터를 생성합니다.
								    var table = $("<table></table>").addClass("table table-responsive table-bordered text-center");
								    var tableHead = $("<thead></thead>");
								    var tableBody = $("<tbody></tbody>");
								    var tableHeadRow = $("<tr></tr>").css("background-color", "#eeeeee");
								    // 테이블 헤드 컬럼을 생성합니다.
								    tableHeadRow.append($("<th>#</th>"));
								    tableHeadRow.append($("<th>시작연도</th>"));
								    tableHeadRow.append($("<th>시작학기</th>"));
								    tableHeadRow.append($("<th>종료연도</th>"));
								    tableHeadRow.append($("<th>종료학기</th>"));
								    tableHeadRow.append($("<th>신청일</th>"));
								    tableHeadRow.append($("<th>상태</th>"));
								    tableHead.append(tableHeadRow);
								    // 테이블 바디 데이터를 생성합니다.
								    $.each(res, function (index, item) {
										let unixTime = item.acReportDate; // Unix 시간
								      ...
								      tableBodyRow.append($("<td>" + index + "</td>"));
								      tableBodyRow.append($("<td>" + item.acStartYear + "</td>"));
								      tableBodyRow.append($("<td>" + item.acStartSmstr + "</td>"));
								      tableBodyRow.append($("<td>" + item.acEndYear + "</td>"));
								      tableBodyRow.append($("<td>" + item.acEndSmstr + "</td>"));
								      tableBodyRow.append($("<td>" + formattedDate + "</td>"));
								      var status = "";
								      if (item.acStatus == "W") {
								        status = "<span class='badge light badge-warning'>미승인</span>";
								      } else if (item.acStatus == "Y") {
								        status = "<span class='badge light badge-success'>승인</span>";
								      } else {
								        status = "<span class='badge light badge-warning'>반려</span>";
								      }
								      tableBodyRow.append($("<td>" + status + "</td>"));
