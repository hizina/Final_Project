...
public class ClassRoomController {
	...
	public String classRoomList(HttpServletRequest req, Model model) {
		...
		if(session != null && session.getAttribute("userInfo") != null) {
			...
			CurrentInfoVO currentInfo = service.getCurrentInfo();
	...
	public String proClassRoom(@PathVariable String lecCode,AtdVO atdVO ,Model model) {
		...
		// 시험 리스트
		List<ExamVO> examList = examService.selectExamInfoInClassRoom(lecCode);
		log.info("examList: " + examList);
		
		List<NoticeVO> noticeList = nService.selectNoticeList(lecCode);
